...
import { saveEvidenceDraft } from "@/features/submissions/submissions.actions";
import type {
  EvidenceDraft,
  EvidenceDraftLink,
  EvidenceDraftState,
...
export function EvidenceDraftForm({ draft }: { draft: EvidenceDraft }) {
  ...
  const [state, action, pending] = useActionState(saveEvidenceDraft, initialState);
    ...
    <section className="mt-8 border-t border-stone-200 pt-6" aria-labelledby="evidence-draft-title">
      ...
      <form action={action} className="mt-6 space-y-6" noValidate>
          ...
          {links.map((link, index) => {
            ...
            return (
              <fieldset key={link.key} className="rounded-md border border-stone-300 p-4">
                ...
                <button type="button" onClick={() => setLinks((current) => current.filter((item) => item.key !== link.key))} className="mt-4 inline-flex min-h-11 items-center rounded-sm border-2 border-ink px-4 py-2 font-semibold text-ink hover:bg-stone-100">
        ...
        <button type="submit" disabled={pending} className="inline-flex min-h-11 w-full items-center justify-center rounded-sm border-2 border-ink bg-launch px-5 py-2.5 font-semibold text-ink transition-transform hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:transform-none md:w-auto">
          {pending ? "Зачувуваме…" : "Зачувај draft"}
        </button>
      </form>
    </section>
  );
