...
export function OnboardingForm({ initialValues, minimumDate, maximumDate }: FormProps) {
  ...
  return (
    <form action={action} className="space-y-8" noValidate>
      {state.status === "error" ? <div id="onboarding-error-summary" ref={errorSummaryRef} role="alert" tabIndex={-1} className="rounded-md border border-coral bg-white p-4 text-ink"><p className="font-semibold">Податоците не се зачувани</p><p className="mt-1 text-sm leading-relaxed">{state.message}</p></div> : null}
      ...
      <fieldset className="space-y-5 border-t border-stone-200 pt-8">
      ...
      <aside className="rounded-md border border-stone-300 bg-stone-100 p-4 text-sm leading-relaxed text-stone-700"><h2 className="font-semibold text-ink">Пред да продолжиш</h2><p className="mt-2">Овие податоци ќе ги гледа Стефан за рачен преглед. Не внесувај лозинки, API keys, приватни tokens или лични податоци од интервјуирани лица.</p></aside>
      <button type="submit" disabled={pending} className="inline-flex min-h-11 w-full items-center justify-center rounded-sm border-2 border-ink bg-launch px-5 py-2.5 font-semibold text-ink transition-transform hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:transform-none md:w-auto">{pending ? "Зачувуваме…" : "Зачувај и продолжи"}</button>
    </form>
  );
}
