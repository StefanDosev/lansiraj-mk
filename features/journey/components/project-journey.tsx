...
export function ProjectJourney({ journey }: Readonly<{ journey: ProjectJourneyModel }>) {
  return (
    <section className="bg-white p-5 text-ink md:p-7" aria-labelledby="journey-heading">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b-2 border-ink pb-6">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cobalt">Проектна патека</p>
          <h2 id="journey-heading" className="mt-3 font-display text-3xl font-semibold md:text-4xl">Од идеја до јавен производ</h2>
        </div>
        <p className="text-sm font-semibold text-stone-700">{journey.approvedCount} од {journey.taskCount} задачи се одобрени</p>
      </div>

      <ol className="mt-8 grid gap-0 lg:grid-cols-6" aria-label="Шест фази на проектната патека">
      ...
      <div className="mt-10 border-t-2 border-ink pt-7">
        ...
        <ol className="mt-4 divide-y divide-stone-200">
          {journey.stages.flatMap((stage) => stage.tasks).map((task) => (
            <li key={task.slug} className="grid gap-3 py-5 sm:grid-cols-[3rem_1fr_auto] sm:items-start">
              <span className={`flex h-8 w-8 items-center justify-center border text-sm font-semibold ${markerClasses[task.state]}`} aria-hidden="true">
      ...
      <div className="mt-8 border-t-2 border-ink pt-6" aria-labelledby="live-endpoint-heading">
        ...
        {journey.endpoint.kind === "live" ? (
          ...
          <div className="mt-3 border-l-4 border-stone-300 bg-stone-100 p-4">
          ...
          <p className="mt-3 border-l-4 border-coral bg-stone-100 p-4 text-sm leading-relaxed" role="alert">{journey.endpoint.message}</p>
        )}
      </div>
    </section>
  );
