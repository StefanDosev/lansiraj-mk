...
export function ScopeAssessmentForm({ projectId, assessment }: { projectId: string; assessment: ProjectScopeAssessment | null }) {
  ...
  return (
    <form action={action} className="rounded-md border border-stone-700 bg-ink p-5 md:p-6" noValidate>
      ...
      {state.status !== "idle" ? <div ref={statusRef} role={state.status === "error" ? "alert" : "status"} tabIndex={state.status === "error" ? -1 : undefined} className={`mt-5 rounded-md border p-4 text-sm ${state.status === "error" ? "border-coral text-white" : "border-acid text-white"}`}>{state.message}</div> : null}
      <fieldset className="mt-6 space-y-3">
        <legend className="font-semibold text-white">Одлука</legend>
        {(["ready", "needs_reduction"] as const).map((readiness) => <label key={readiness} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-md border border-stone-700 px-4 py-3 text-white has-checked:border-acid"><input type="radio" name="readiness" value={readiness} defaultChecked={state.values.readiness === readiness} className="h-5 w-5 accent-acid" /><span>{readiness === "ready" ? "Подготвен опсег" : "Потребно е намалување"}</span></label>)}
      ...
      <button type="submit" disabled={pending} className="mt-6 inline-flex min-h-11 items-center justify-center rounded-sm border-2 border-white bg-acid px-5 py-2.5 font-semibold text-ink transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:transform-none">{pending ? "Зачувуваме…" : "Зачувај проценка"}</button>
    </form>
  );
}
